...
for c in cases["spkezCases"]:
    try:
        state, lt = spice.spkez(c["target"], c["et"], "J2000", c["abcorr"], c["center"])
...
with open(os.path.join(FIXTURES, "results-py.json"), "w") as f:
    json.dump({"str2etResults": str2et_results, "spkezResults": spkez_results}, f, indent=2)

print(
    f"spiceypy: {len(str2et_results)} str2et cases, {len(spkez_results)} spkez cases -> results-py.json "
